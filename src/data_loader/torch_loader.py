...
"""
from src.data_loader.loader import LoadData
...
class TorchLoader(LoadData):
    ...
    def load_files(self):
        ...
        if self._x_train_set == [] and self._y_train_set == []:
        ...
        if self._x_val_set == [] and self._y_val_set == []:
        ...
        if self._x_test_set == [] and self._y_test_set == []:
    ...
    def load_data(self, source_path):
        ...
        :return: self
        """
        if path.exists("{}/training_set_t_{}.npz".format(source_path, self._sample_rate)):
            tr = np.load("{}/training_set_t_{}.npz".format(source_path, self._sample_rate), allow_pickle=True)
        ...
        if path.exists("{}/validation_set_t_{}.npz".format(source_path, self._sample_rate)):
            tr = np.load("{}/validation_set_t_{}.npz".format(source_path, self._sample_rate), allow_pickle=True)
